...
static String:DuelNames[4][16] 					= {"关闭", "正常", "时间", "击杀数"};
static String:ClassNames[TFClassType][] 		= {"所有", "侦察兵", "狙击手", "士兵", "爆破手", "医生", "机枪手", "火焰兵", "间谍", "工程师" };
...
public DuelOption(Player1)
{
	...
	SetPanelTitle(menu, "决斗选项:");
	Format(MenuItem, sizeof(MenuItem), "决斗                        [%s]", DuelNames[g_Duel[Player1][Type]]);
	DrawPanelItem(menu, MenuItem);
	
	if(g_Duel[Player1][Type] == 2)
	{
		Format(MenuItem, sizeof(MenuItem), "时间                        [%i %s]", g_Duel[Player1][TimeLeft], g_Duel[Player1][TimeLeft] >1 ? "时间":"min");
		DrawPanelItem(menu, MenuItem);
	}
	else if(g_Duel[Player1][Type] == 3)
	{
		Format(MenuItem, sizeof(MenuItem), "个数                   [%i %s]", g_Duel[Player1][Score], g_Duel[Player1][Score] >1 ? "击杀数":"kill");
		DrawPanelItem(menu, MenuItem);
	}
	else
		DrawPanelText(menu, " ");
	
	Format(MenuItem, sizeof(MenuItem), "固定职业                [%s]", ClassNames[g_Duel[Player1][ClassRestrict]]);
	DrawPanelItem(menu, MenuItem);
	Format(MenuItem, sizeof(MenuItem), "挑战者保护       [%s]", g_Duel[Player1][GodMod] ? "ON":"OFF");
	DrawPanelItem(menu, MenuItem);
	Format(MenuItem, sizeof(MenuItem), "爆头击杀                  [%s]", g_Duel[Player1][HeadShot] ? "ON":"OFF");
	DrawPanelItem(menu, MenuItem);
	DrawPanelText(menu, " ");
	DrawPanelItem(menu, " ");
	DrawPanelItem(menu, "发送请求");
...
public ChallengerMenu(Player1, Player2)
{
	...
	Format(MenuTitle, sizeof(MenuTitle), "%N 挑战了你!", Player1);
	SetPanelTitle(menu, MenuTitle);
	if(g_Duel[Player1][Type] == 1)
		Format(MenuItem, sizeof(MenuItem), "类型:普通");
	else if(g_Duel[Player1][Type] == 2)
		Format(MenuItem, sizeof(MenuItem), "类型:时间         [%i %s]", g_Duel[Player1][TimeLeft], g_Duel[Player1][TimeLeft] >1 ? "时间":"min");
	else if(g_Duel[Player1][Type] == 3)
		Format(MenuItem, sizeof(MenuItem), "类型:击杀数   [%i %s]", g_Duel[Player1][Score], g_Duel[Player1][Score] >1 ? "击杀数":"kill");
	...
	DrawPanelItem(menu, "是的,我接受");
	DrawPanelItem(menu, "不, 我拒绝");
...
HudMessageTime(iClient)
{
	...
	if(g_Duel[iClient][Type] == 1 || g_Duel[iClient][Type] == 3)	ShowHudText(iClient, -1, "你 : %i - 他: %i", g_Duel[iClient][Score], g_Duel[g_Duel[iClient][Challenger]][Score]);
	else if(g_Duel[iClient][Type] == 2)	ShowHudText(iClient, -1, "时间剩下 : %i | 你 : %i - 他: %i", g_Duel[iClient][TimeLeft], g_Duel[iClient][Score], g_Duel[g_Duel[iClient][Challenger]][Score]);
}
...
{	
	...
	SetPanelTitle(rnkpanel, "你的决斗战绩:");
	Format(value, sizeof(value), "名字: %s", ClientID);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "全服排名: %i out of %i", Rank , RankTotal);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "总分数: %f" , points[iClient]);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "胜利: %i" , victories[iClient]);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "总局数: %i" , total[iClient]);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "击杀数: %i" , killsNbr[iClient]);
	DrawPanelText(rnkpanel, value);
	Format(value, sizeof(value), "死亡数: %i" , death[iClient]);
	DrawPanelText(rnkpanel, value);
	DrawPanelItem(rnkpanel, "退出");
