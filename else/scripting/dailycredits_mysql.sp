...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_sign", Cmd_Daily);
	InitializeDB();
}
...
stock void GiveCredits(int client, bool FirstTime)
{
	char buffer[200];
	char steamId[32];
	DailyCredits = GetConVarInt(g_hDailyCredits);
    if (CheckCommandAccess(client, "Give20ExtraHpOrWhatever", ADMFLAG_CUSTOM1, true))
    {
        DailyCredits = DailyCredits + 10;
    }
    if (CheckCommandAccess(client, "Give20ExtraHpOrWhatever", ADMFLAG_CUSTOM2, true))
    {
        DailyCredits = DailyCredits + 10;
    }
	if (GetClientAuthId(client, AuthId_Steam2, steamId, sizeof(steamId)))
	{
		if (FirstTime)
		{
			Store_SetClientCredits(client, Store_GetClientCredits(client) + DailyCredits);
			CPrintToChatEx(client, client, "%t", "CreditsRecieved", DailyCredits);
		...
		else
		{
			...
			if ((date1 - date2) == 1)
			{
				int TotalCredits = DailyCredits + (bonus * GetConVarInt(g_hDailyBonus)); //bonus can't start at 1, since the first day would get the player a bonus as well
