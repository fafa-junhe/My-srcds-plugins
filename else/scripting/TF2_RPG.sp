...
new String:plugin_tag[40] = "{green}[RPG系统]{default}";
...
public OnPluginStart()
{
	...
	for (new i = MaxClients; i > 0; --i)
	{
		if(IsValidClient(i))
		{
			...
			ShowHudText(i, -1, "[ 等级 : %i | 经验 : %i/%i ]", p_level[i] , p_exp[i] , p_exp_level_up[i] );
...
public Action:Event_Changeclass(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(HelpMessageDisplayed[client] == false)
	{
		...
		SetHudTextParams(-1.0, 0.85, 0.5, 0, 255, 0, 200, 0, 0.00001, 0.000001, 0.000001);
		ShowHudText(client, -1, "[ 等级 : %i | 经验 : %i/%i ]", p_level[client] , p_exp[client] , p_exp_level_up[client] );
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(killer != client)
	{
		...
		if(p_exp[killer] >= p_exp_level_up[killer])
		{
			...
			p_level[killer]++;
			CPrintToChatAll("%s %N 升级到了 {green}%i{default} !", plugin_tag, killer, p_level[killer]);
		}
	}
...
public Action:RefreshStat(Handle:timer, any:serial)
{
	...
	ShowHudText(client, -1, "钱 : %i $", p_cash[client]);
...
stock DisplayRPGMenu(client)
{
	...
	switch(p_class)
	{
		...
		case TFClass_Scout : Format(class, sizeof(class), "侦察兵");
		case TFClass_Sniper : Format(class, sizeof(class), "狙击手");
		case TFClass_Soldier : Format(class, sizeof(class), "士兵");
		case TFClass_DemoMan : Format(class, sizeof(class), "爆破手");
		case TFClass_Medic : Format(class, sizeof(class), "医生");
		case TFClass_Heavy : Format(class, sizeof(class), "胖子");
		case TFClass_Pyro : Format(class, sizeof(class), "火焰兵");
		case TFClass_Spy : Format(class, sizeof(class), "间谍");
		case TFClass_Engineer : Format(class, sizeof(class), "工程师");
	}
	Format(menu_title, sizeof(menu_title), "RPG系统 商城 - %s", class);
	...
	if(GetArraySize(Array_SQLreturn) != 0)
	{
		for(new i = 0; i < (GetArraySize(Array_SQLreturn)/6); i++)
		{
			...
			if(StringToInt(effect_time) == -1)
				Format(item, sizeof(item), "%s - [%s $] [等级 %s] [%s/%i]", item_name, list_price[StringToInt(upg_level)], level, upg_level, max_upg_lvl);
			else
				Format(item, sizeof(item), "%s - [%s $] [等级 %s] [%s 秒]", item_name, list_price[StringToInt(upg_level)], level, effect_time);

			if(FindStringInArray(Array_UpgradeInUse[client], id_upgrade) != -1 && StringToInt(effect_time) == -1 && max_upg_lvl == StringToInt(upg_level))
			{
				StrCat(item, sizeof(item), " [已购买]");
...
stock bool:AddUpgradeToPlayer(client, const String:id_upgrade[], bool:bypass_check = false, Handle:UpgradeArray = INVALID_HANDLE)
{
	...
	{
		CPrintToChat(client, "%s 你没有足够的等级购买这个 !", plugin_tag);
		return false;
	}

	if(p_cash[client] < StringToInt(list_price[tab_index]) && bypass_check == false)
	{
		CPrintToChat(client, "%s 你没有足够的钱购买这个 !", plugin_tag);
		return false;
	}
...
public PickedSmall(entity, client)
{
	if(IsValidClient(client))
	{
		p_cash[client] += GetRandomInt(1,2);
...
public PickedLarge(entity, client)
{
	if(IsValidClient(client))
	{
		p_cash[client] += GetRandomInt(7,10);
		AcceptEntityInput(entity, "Kill");
	}
}
