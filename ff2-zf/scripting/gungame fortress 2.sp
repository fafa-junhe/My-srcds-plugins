...
UpgradePlayer(iClient)
{
	...
	else if (gg_iRank[iClient] < MaxRank)
	{
		...
		CPrintToChat(iClient, "{olive}[GG]{default} {olive}You upgraded to rank: {green}%d/%d", gg_iRank[iClient]+1, MaxRank+1);
		LogMessage("%N upgraded to rank: %d/%d", iClient, gg_iRank[iClient]+1, MaxRank+1);
		new iMax = GetMaxRank();
		if (gg_iRank[iClient] == iMax && GetRankCount(iMax) <= 1)
		{
		   CPrintToChatAllEx(iClient, "{olive}[GG]{default} {olive}The leader is {teamcolor}%N{olive} with rank {green} %d/%d", iClient, iMax+1, MaxRank+1);
		}
	}
...
GunGameWeapons(iClient)
{
	...
	if (iClass != WeaponClass[gg_iRank[iClient]])
	{
		...
		{
			LogError("[GunGame_Fortress2] Invalid Class !");
			LogMessage("Can't Upgraded, invalid class RANK : %d , ITEM : %d ",gg_iRank[iClient], WeaponClass[gg_iRank[iClient]] );
		}
	}
	TF2_RemoveAllWeapons(iClient);
	if(item[gg_iRank[iClient]] < 0 )
	{
		LogError("[GunGame_Fortress2] Invalid Weapon !");
		LogMessage("Can't Upgraded, invalid weapon RANK : %d , ITEM : %d ",gg_iRank[iClient], item[gg_iRank[iClient]] );
	}
	else 
	{
...
GunGameDeath(iClient)
{
	...
	if (gg_iDeathCount[iClient] >= 5)
	{
		...
		if (gg_iRank[iClient] > 0)
		{
			...
			CPrintToChat(iClient, "{olive}[GG] {red}You've downgraded to:{green} %d/%d{red},{black} for 5 deads and never kill ! ", gg_iRank[iClient]+1, MaxRank+1);
...
GunGameBonus(iClient)
{
	...
	{
		CPrintToChat(iClient, "{olive}[GG] {orange}Bonus, 5 kills in a row : {green}+200 healts !");
...
stock AddToClientWinPoints(client)
{
	...
	if (points <= 1) CPrintToChatAllEx(client, "{olive}[GG] {orange}The winner is: {teamcolor}%N{orange}. This is their {green}first {orange}victory!", client);
	else CPrintToChatAllEx(client, "{olive}[GG] {orange}The winner is: {teamcolor}%N{orange}. They've won {green}%d {orange}times!!", client, points);
