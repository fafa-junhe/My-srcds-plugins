...
// WORK
// (Round Start Respawn)    [VSH-ZF] event_PlayerSpawn 1 6
//                          [VSH-ZF] Grace period begun. Survivors can change classes.
// (Round Start Spawn)      [VSH-ZF] spawnClient 1 6
// (Spawn from spawnClient) [VSH-ZF] event_PlayerSpawn 1 5
// (Postspawn from spawn 1) [VSH-ZF] timer_postSpawn 1 5
// (Postspawn from spawn 2) [VSH-ZF] timer_postSpawn 1 5
...
{ 
  ...
  PrintToChatAll("\x05[VSH-ZF]\x01 ZF 初始化中~");
...
public Action:command_zfDisable (client, args)
{
  ...
  zfDisable();
  ServerCommand("mp_restartgame 10");  
  PrintToChatAll("\x05[VSH-ZF]\x01 ZF 卸载中~");
...
public Action:command_zfSwapTeams(client, args)
{
  ...
  zfSwapTeams();
  ServerCommand("mp_restartgame 10");
  PrintToChatAll("\x05[VSH-ZF]\x01 Team roles swapped. Restarting Round...");
...
public Action:event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  else
  {
    ...
    PrintToChatAll("\x05[VSH-ZF]\x01 准备时间开始(幸存者可切换兵种和技能).");  
  }
...
public Action:event_SetupEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  if(roundState() != RoundActive)
  {
    ...
    PrintToChatAll("\x05[VSH-ZF]\x01 准备时间结束");
...
//   // DEBUG
//   PrintToChat(client, "[VSH-ZF] event_PlayerSpawn %d %d", client, _:TF2_GetPlayerClass(client));
...
{
//   // DEBUG
//   PrintToChat(client, "[VSH-ZF] timer_postSpawn %d %d", client, _:TF2_GetPlayerClass(client));
...
{   
  ...
  {
    PrintToChat(client, "\x05[VSH-ZF]\x01 你被感染了....");
    PrintToChat(client, "\x05[VSH-ZF]\x01 然而这是VSH版ZF 所以你没有去蓝队 ");
  }
...
public help_printZFInfoChat(client)
{
  if(client == 0)
  {
    PrintToChatAll("\x05[VSH-ZF]\x01 输入 \"!zf\" 打开菜单!");
  }
  else
  {
    PrintToChatAll("\x05[VSH-ZF]\x01 输入 \"!zf\" 打开菜单!");
