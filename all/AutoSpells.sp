...
{
	CreateConVar("sm_spell_version", VERSION, "Spell Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	v_RespawnTime = CreateConVar("sm_spell_respawntime", "20.0", "How quickly spells respawn.", 0, true, -1.0);
	HookConVarChange(v_RespawnTime, UpdateCvar);

	RegAdminCmd("sm_placespell", SeedPumpkin, ADMFLAG_ROOT);
	RegAdminCmd("sm_createspell", SeedPumpkin, ADMFLAG_ROOT);
	RegAdminCmd("sm_spawnspell", SeedPumpkin, ADMFLAG_ROOT);
	RegAdminCmd("sm_seedspell", SeedPumpkin, ADMFLAG_ROOT);
	RegAdminCmd("sm_deletespell", DelPumpkin, ADMFLAG_ROOT);
...
public Action:SeedPumpkin(client, args)
{
	...
	{
		PrintToChat(client, "\x04[\x03AutoSpell\x04]\x01 Entity limit is reached. Can't spawn anymore Spells. Change Map(s).");
		return Plugin_Handled;
	}
	
	new iPumpkin = CreateEntityByName("tf_spell_pickup");
	
	if(IsValidEntity(iPumpkin))
	{		
		PrintToChat(client, "\x04[\x03AutoSpell\x04]\x01 Spell location added to database!")
		new Float:pos[3];
...
public Action:DelPumpkin(client, args)
{
	...
	if (iEntity != -1 && iEntity != -2)
	{
		...
		PrintToChat(client, "\x04[\x03AútoSpell\x04]\x01 Spell location deleted from database!")
