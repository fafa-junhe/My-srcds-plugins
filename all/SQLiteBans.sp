...
public Action:Command_AddBan(client, args)
{
	...
	if(Duration == 0)
	{
		ShowActivity2(client, "[SM] ", "added a permanent ban on identity. Reason: %s", BanReason);
		LogSQLiteBans("Admin %N [AuthId: %s] added a permanent ban on identity. Reason: %s", client, AdminAuthId, TargetArg);
	}
	else
	{
